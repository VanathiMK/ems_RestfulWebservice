...
const EmployeeModel=model.EmpModel;
function getNextID(){
    var nextID;
    try{
...
exports.createOrSave=async(request,response)=>{
    ...
    try{
        await EmployeeModel.create(emp1).then(employee => {
...
try{ 
    ...
    console.log(id);
    await  EmployeeModel.get(id).then(employee =>{
        if(employee != null){
...
exports.deleteEmployee=async(request,response)=>{
 ...
 try{
    ...
    await EmployeeModel.delete(id).then(()=>{
...
try{
 ...
 console.log("id   "+id2);
    await EmployeeModel.update(emp).then(employee=>{
